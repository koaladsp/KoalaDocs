...
[Artifact Signing](https://azure.microsoft.com/en-us/products/artifact-signing/) is a new end-to-end code signing service which is currently available to the general public as part of Microsoft Azure.
...
1. Artifact Signing - an Azure service aimed specifically at code signing, which we can recommend.
2. Azure Key Vault - an Azure service that provides a remote HSM.

As the Azure Key Vault is a replacement for HSM use cases, will cover the use of the much easier **Artifact Signing** in this guide instead.

## 2. Artifact Signing

⚠ Please note that there are currently at least two ways of using Artifact Signing:

1. The **_official_** `signtool.exe` and its [Artifact Signing Client Dlib](https://learn.microsoft.com/en-us/azure/artifact-signing/how-to-signing-integrations) as supported by Microsoft.
...
The way the official Artifact Signing tool works is by extending the existing Microsoft supplied `signtool.exe` with a dynamic library for Azure support. The connection to Azure is used to create and use an ad hoc certificate which is then used to sign a binary. These certificates are issued by Microsoft. In fact, Microsoft itself acts as a certificate authority with a root certificate chain already preinstalled in modern versions of Microsoft Windows.

Unlike certificates issued from authorities such as Sectigo, DigiCert and others, the certificates issued by Artifact Signing are only valid for a short time (e.g. several days) so that leaked certificates cannot be abused easily by malicious actors. To be fair, the new requirement for a HSM to store these certificates, instead of a file, also makes this much less likely but still.
...
#### 2.2.1. Creating a Artifact Signing Account
...
Now that the Artifact Signing resources have been set up, we need to set up Artifact Signing so it can issue new certificates for code signing.
...
## 2.3. Setting up Azure Artifact Signing for code signing

Now that Artifact Signing has been configured in the Microsoft Azure portal, we can set up the code signing tools on our local machine or automated build machine.
...
We will need to install the Azure Command-Line Interface tools in order to make `signtool.exe` work with Artifact Signing. This only needs to be done once. The CLI tools will be installed on your system, and will be accessible from a command prompt afterwards.
...
### 2.3.2. Installing `signtool.exe` and Artifact Signing Dlib

`signtool.exe` is the **_official_** code signing tools distributed by Microsoft with Windows SDKs.

⚠ You should assume that the required version of `signtool.exe` is _not_ installed on your system. On older versions, Artifact Signing will not work, and will not show meaningful errors. Furthermore, the [official docs](https://learn.microsoft.com/en-us/azure/artifact-signing/how-to-signing-integrations) on minimum SDK requirements _contain the wrong version numbers_ so best to disregard them. So make sure to follow the installation instructions here.
...
3. Let's install both the correct `signtool.exe` as well as the Artifact Signing Dlib:
   ```dos
...
The Artifact Signing Dlib should be installed in a directory that is or starts with `Microsoft.ArtifactSigning.Client` such as: `Microsoft.ArtifactSigning.Client.1.0.115`. Make sure to locate the `Azure.CodeSigning.Dlib.dll` file, e.g. `C:\sign\Microsoft.ArtifactSigning.Client.1.0.115\bin\x64\Azure.CodeSigning.Dlib.dll` and double check that the platform directory (`x86` or `x64`) matches with the signtool's path above.
...
We now need to create a metadata configuration JSON file to make the signtool work with Artifact Signing. This JSON file can be placed anywhere you like.
...
* Create a `metadata.json` file with the following contents (replace details accordingly):
	...
	```
	* Endpoint URL: choose according to the endpoint you've chosen for the Azure Artifact Signing:
		* East US (https://eus.codesigning.azure.net)
		* West US (https://wus.codesigning.azure.net)
		* West Central US (https://wcus.codesigning.azure.net)
		* West US 2 (https://wus2.codesigning.azure.net)
...
set ACS_DLIB=C:\sign\Microsoft.ArtifactSigning.Client.1.0.115\bin\x64\Azure.CodeSigning.Dlib.dll
set ACS_JSON=C:\sign\metadata.json
```

### 2.3.4. Testing `signtool.exe`

We now need to make sure that `signtool.exe` is working and capable of using Artifact Signing to sign executables, before doing anything else.
...
Make sure to use the absolute path to `signtool.exe` that you noted down earlier. Also replace the `filetobesigned.exe` with the executable that you want to sign. When you run this, you should immediately see messages involving Artifact Signing, and your signing should be successful.
...
Now that signtool is successfully using Artifact Signing to sign executables, we can now proceed to making signtool with Artifact Signing work with PACE's wraptool. The PACE wraptool relies on signtool for its certificates, but as of the latest update of this guide _does not support_ the official Microsoft signtool with the Artifact Signing Dlib. It _does_ support the _unofficial_ AzureSignTool, but as mentioned before, this is not the focus of this guide.
...
The normal flow for wraptool relies on either a certificate file and password (no longer possible due to HSM) or a sign id (HSM) to passed in by the developer, with no apparent support for any other tools or options. However, we can use utility scripts and let the wrap tool use these instead to injects all the necessary arguments for Artifact Signing to work.
...
2. Create a batch file called `aax-signtool.bat` with the following contents in a directory of your choice:
	...
	:: which are incompatible with Artifact Signing.
	::
...
If you're curious as to what these utility scripts do, the purpose of these two scripts is:
	...
	2. To inject the proper command-line arguments to Microsoft's sign tool, along with the original arguments from the PACE wrap tool, so that Artifact Signing will be used instead of the regular HSM code signing.
...
In any case, you should now be able to run `wraptool.exe` such that it invokes `signtool.exe` making use of Artifact Signing. In order to this, you will need to pass the following arguments, e.g.:
...
You should now be able to code sign and wrap binaries using Artifact Signing. Remember to use `wraptool.exe sign` for code signing AAX plugins, and `wraptool.exe wrap` for wrapping and code signing binaries such as standalone applications.

## 3. Final words

With any luck, you should now have Artifact Signing working with `signtool.exe` to sign application binaries, as well as with `wraptool.exe` to hopefully sign and/or wrap binaries.
